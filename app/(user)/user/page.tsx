...
export default async function UserPage() {
    ...
    const reservasAtivas = reservations.filter(reservation => reservation.status === 'PENDING' || reservation.status === 'CONFIRMED' || reservation.status === 'CHECKED_IN')
    const nextReservation = calculateNextReservation(reservasAtivas)
    ...
    return (
        <div>
            ...
            <div className="flex gap-[1rem] items-center">
                <CardInfo icon={<FaBed className="w-7 h-7"/>} label="RESERVAS ATIVAS" value={reservasAtivas.length.toString()} />
